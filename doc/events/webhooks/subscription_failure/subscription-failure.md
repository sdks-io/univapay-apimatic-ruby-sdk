...
```ruby
# Implementation example of handling the `subscriptionFailure` event (with no signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    handler = SubscriptionFailureHandler.new
    ...
    if event.is_a?(SubscriptionFailureWebhookCallback) && event.event == 'subscription_failure'
      puts 'SubscriptionFailure received'
      # TODO: Add subscriptionfailure handling
