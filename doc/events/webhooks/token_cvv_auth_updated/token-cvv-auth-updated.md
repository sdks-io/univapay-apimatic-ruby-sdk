...
```ruby
# Implementation example of handling the `tokenCvvAuthUpdated` event (with no signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(TokenCvvAuthUpdatedWebhookCallback) && event.event == 'token_cvv_auth_updated'
      puts 'TokenCvvAuthUpdated received'
      # TODO: Add tokencvvauthupdated handling
