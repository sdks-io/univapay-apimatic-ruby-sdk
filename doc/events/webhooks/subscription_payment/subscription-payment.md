...
```ruby
# Implementation example of handling the `subscriptionPayment` event (with no signature verification) in Rails.
...
class WebhooksController < ActionController::API
  def receive
    ...
    if event.is_a?(SubscriptionPaymentWebhookCallback) && event.event == 'subscription_payment'
