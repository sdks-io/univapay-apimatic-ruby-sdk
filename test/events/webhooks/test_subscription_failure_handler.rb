...
class TestSubscriptionFailureHandler < Minitest::Test
  parallelize_me!
...
def make_request(body_obj, add_signature_header: true)
  ...
  def test_subscription_failure_from_subscription_failure_handler
    ...
    # assert
    assert event.is_a?(SubscriptionFailureWebhookCallback) && event.event == 'subscription_failure'
  end
