...
class TestTokenCreatedHandler < Minitest::Test
  parallelize_me!
...
def make_request(body_obj, add_signature_header: true)
  ...
  def test_token_created_from_token_created_handler
    ...
    # assert
    assert event.is_a?(TokenCreatedWebhookCallback) && event.event == 'token_created'
  end
