...
class TestSubscriptionCanceledHandler < Minitest::Test
  parallelize_me!
...
def make_request(body_obj, add_signature_header: true)
  ...
  def test_subscription_canceled_from_subscription_canceled_handler
    ...
    # assert
    assert event.is_a?(SubscriptionCanceledWebhookCallback) && event.event == 'subscription_canceled'
  end
