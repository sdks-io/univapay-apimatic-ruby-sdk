...
class TestSubscriptionCompletedHandler < Minitest::Test
  parallelize_me!
  include UnivapayClientSdk
  include CoreLibrary

def setup
  @handler = SubscriptionCompletedHandler.new
end
...
def make_request(body_obj, add_signature_header: true)
  ...
  def test_subscription_completed_from_subscription_completed_handler
    ...
    # assert
    assert event.is_a?(SubscriptionCompletedWebhookCallback) && event.event == 'subscription_completed'
  end
