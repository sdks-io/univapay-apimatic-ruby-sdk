...
class TestSubscriptionCreatedHandler < Minitest::Test
  parallelize_me!
  include UnivapayClientSdk
  include CoreLibrary

def setup
  @handler = SubscriptionCreatedHandler.new
end
...
def make_request(body_obj, add_signature_header: true)
  ...
  def test_subscription_created_from_subscription_created_handler
    ...
    # assert
    assert event.is_a?(SubscriptionCreatedWebhookCallback) && event.event == 'subscription_created'
  end
