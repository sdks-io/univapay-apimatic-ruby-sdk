...
class TestSubscriptionSuspendedHandler < Minitest::Test
  parallelize_me!
...
def make_request(body_obj, add_signature_header: true)
  ...
  def test_subscription_suspended_from_subscription_suspended_handler
    ...
    # assert
    assert event.is_a?(SubscriptionSuspendedWebhookCallback) && event.event == 'subscription_suspended'
  end
