...
class TestTokenCvvAuthCheckUpdatedHandler < Minitest::Test
  parallelize_me!
...
def make_request(body_obj, add_signature_header: true)
  ...
  def test_token_cvv_auth_check_updated_from_token_cvv_auth_check_updated_handler
    # Tests the `tokenCvvAuthCheckUpdated` event from tokenCvvAuthCheckUpdatedHandler.
    # arrange
    event_payload = {
    ...
    # assert
    assert event.is_a?(TokenCvvAuthCheckUpdatedWebhookCallback) && event.event == 'token_cvv_auth_check_updated'
  end
