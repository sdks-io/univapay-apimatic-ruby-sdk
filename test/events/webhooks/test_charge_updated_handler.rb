...
class TestChargeUpdatedHandler < Minitest::Test
  parallelize_me!
...
def make_request(body_obj, add_signature_header: true)
  ...
  def test_charge_updated_from_charge_updated_handler
    ...
    # assert
    assert event.is_a?(ChargeUpdatedWebhookCallback) && event.event == 'charge_updated'
  end
